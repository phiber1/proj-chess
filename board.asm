...
NODE_IN_CHECK   EQU $6742   ; 8 bytes - per-ply "is this node in check" snapshot
                            ; ($6742-$6749). Snapshotted at NEGAMAX entry from
...
; ------------------------------------------------------------------------------
; Static Exchange Evaluation (SEE) scratch: $674A-$6775
; ------------------------------------------------------------------------------
; Used by SEE_CORRECT_STANDPAT at the QS leaf to resolve the full capture
; exchange on UNDO_TO statically (no recursion). Reclaimed from the dead
; QSEARCH capture loop + its QS_MOVE_LIST ($6780-$67FF, now unused).
; Live only during a SEE call at a leaf; never overlaps live search state.
; Piece values are /4-scaled bytes (P25 N80 B82 R125 Q225 K250).
...
; Phase gate: SEE runs ONLY when EG_PIECE_COUNT < SEE_ENDGAME_PIECES (the
; endgame, where conversion shuffles happen AND few pieces => SEE is cheap).
; In the opening/middlegame SEE is off, so search timing is unchanged there.
; Matches the eval endgame-block threshold (12). Lower this if endgame d5 is slow.
SEE_ENDGAME_PIECES EQU 12   ; non-king piece count; run SEE below this
