...
;The following source code assumes an 1802 clock speed of 1.78 to 1.8Mhz ONLY
;Written by Charles James Yakym 2015, aka Chuck Yakym
;
;
;Enjoy.... 
;
...
;----------------------------------------------
;9600 BAUD ONLY INPUT ROUTINE - NORMAL LOGIC

;RB.0 = INPUTTED CHARACTER
;RE.0 = DELAY COUNTER = 2 AT 9600 BAUD

NB96IN	BN3	NB96IN	;WAIT FOR STOP BIT 

	LDI	0FFH	;INITIALIZE INPUT CHARACTER TO FFh
	PLO	RB	;STORE IT IN RB.0
	GLO	RE	;GET DELAY COUNT

NB96IN1	B3  NB96IN1	;WAIT FOR START BIT 

	SHR		;DELAY COUNTER DEVIDED BY 2 (.5 BIT TIME VALUE)
	SKP		;SKIP TO DELAY


NB96IN2	GLO	RE	;GET DELAY COUNT
NB96IN3	SMI	01H	;DECREMENT DELAY COUNT
	BNZ	NB96IN3	;WHEN DONE WITH DELAY D=0 AND DF=1

	BN3	NB96IN4	;GET NEXT BYTE
	SKP		;IF BIT=0 (EF3 PIN HIGH), LEAVE DF=0
NB96IN4	SHR		;IF BIT=1 (EF3 PIN LOW), SET DF=1
	GLO	RB	;GET INCOMING BYTE
	SHRC		;RING SHIFT BIT RIGHT
	PLO	RB	;STORE BYTE IN RB.0
	LBDF	NB96IN2	; if DF=1, then Start bit hasn't shifted all
			;    the way thru byte. Loop until it does

			;DELAY BETWEEN LAST BIT AND THE STOP BIT
	GLO	RE	; 
NB96IN5	SMI 1		;DECREMENT DELAY COUNT
	BNZ NB96IN5	; 
...
;SEND START BIT
;ENTRY POINT FOR THE OUTPUT ROUTINE
NB96OUT	GLO	RB	; LOAD D WITH RB.0
	STR	R2	; PUSH RB,0 ONTO THE STACK
	DEC	R2
	GLO	RF	; LOAD D WITH RD.0
	STR	R2	; PUSH RD.0 ONTO THE STACK
	DEC	R2
...
NSTBIT	SEQ		;1	START BIT
	...
	NOP		;9.5 INSTUCTIONS SINCE START BIT
			...
			;DETERMINE FIRST BIT AND OUTPUT IT
	BDF	NSTBIT1	;DF = 1, IF BIT IS LOW THEN JUMP TO NSTBIT1
	BR	NQLO		;JUMP AT 11.5 INSTRUCTION TIME, Q=OFF

NSTBIT1	BR	NQHI		;JUMP AT 11.5 INSTRUCTION TIME, Q=ON



NQLO1	DEC	RF
	GLO	RF
	BZ	NDONE96	;AT 8.5 INSTRUCTIONS EITHER DONE OR REQ

;DELAY
	GLO	RE
NLDELAY	SMI	01H
	BZ	NQLO	;IF DELAY IS DONE THEN TURN Q OFF
			;ADJUST DELAY FOR LOWER BAUD RATES
			;WASTE 9.5 INSTRUCTION TIMES
	NOP		;1.5
	NOP		;3
	...
	NOP		;7.5
	SEX	R2	;8.5
	BR	NLDELAY	;AT 9.5 INSTRUCTION TIMES JUMP TO LDELAY

NQLO	SEQ		;Q OFF
	GLO	RB
	SHRC		;PUT NEXT BIT IN DF
	PLO	RB
	LBNF	NQLO1	;5.5 TURN Q OFF AFTER 6 MORE INSTRUCTION TIMES

NQHI1	DEC	RF
	GLO	RF
	BZ	NDONE96	;AT 8.5 INSTRUCTIONS EITHER DONE OR SEQ

;DELAY
	GLO	RE
NHDELAY	SMI	01H
	BZ	NQHI	;IF DELAY IS DONE THEN TURN Q ON
			;ADJUST DELAY FOR LOWER BAUD RATES
			;WASTE 9.5 INSTRUCTION TIMES
	NOP		;1.5
	NOP		;3 
	...
	NOP		;7.5
	SEX	R2	;8.5
	BR	NHDELAY	;AT 9.5 INSTRUCTION TIMES JUMP TO HDELAY
...
NQHI	REQ		;Q ON
	...
	SHRC		;PUT NEXT BIT IN DF
	PLO	RB
	LBDF	NQHI1	;5.5 TURN Q ON AFTER 6 MORE INSTRUCTION TIMES
...
NXDELAY	SMI	01H
	...
	NOP		;7.5
	SEX	R2	;8.5
	BR	NXDELAY	;AT 9.5 INSTRUCTION TIMES JUMP TO XDELAY
...
NDNE961	REQ		;1 SEND STOP BIT
	...
	SMI	01H	;10.5
	BNZ	NDNE961	;11.5
			;NOTE - STOP BIT IS 3 INSTRUCTION TIMES LONGER THEN NEEDED
			;	PLUS THE RETURN TO CALLER TIME
